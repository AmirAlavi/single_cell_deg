...
LoadData <- function(data.dir) {
    ...
    print(dim(data.env$counts.mat))
    print(dim(data.env$gene.symbols))
...
RunLimma <- function(counts, design, data.env) {
    ## Call limma functions to compute differential expression
    cat("\t", "\t", "Applying voom transform...", "\n")
    t0 <- proc.time()
    ...
    cat("\t", "\t", "Computing moderated test statistics via emperical Bayes...", "\n")
    t0 <- proc.time()
    fit2 <- eBayes(fit)
    print(proc.time() - t0)
    ...
    results <- topTable(fit2, coef="CURvsOTHER", adjust.method="BH", p.value=1, number=Inf)
    print(proc.time() - t0)
    cat("dim(results): ", dim(results), "\n")
    ##processed.results <- CalcStats(ediff, data.env)
    original.colnames <- colnames(results)
    processed.results <- cbind(names(data.env$gene.symbols), data.env$gene.symbols, results)
    cat("dim(processed.results): ", dim(processed.results), "\n")
    new.colnames <- c("EntrezID", "Symbol", original.colnames)
    colnames(processed.results) <- new.colnames
    print(str(processed.results))
    print(head(processed.results))
    return(processed.results)
}

SetUpAndRunLimma <- function(cell.type, cell.type.sv, other.types.sv, data.env) {
    ...
    design <- cbind(OTHER=1, CURvsOTHER=rep(0:1, c(ncol(other.types.counts), ncol(cell.type.counts))))
    results <- RunLimma(combined.counts, design, data.env)
    return(results)
}
...
MultipleDegExperiments <- function(cell.type, accns, data.env) {
    ...
    for (accn in accns) {
        ...
        results <- SetUpAndRunLimma(cell.type, cell.type.and.accn.sv, other.types.sv, data.env)
        ## save results
...
SingleDegExperiment <- function(cell.type, data.env) {
    ...
    result <- SetUpAndRunLimma(cell.type, cell.type.sv, other.types.sv, data.env)
    ## save results
...
AnalyzeDegResults <- function(deg.results, data.env) {
    ...
    for (results in deg.results) {
        max.adj.pvals <- pmax(max.adj.pvals, results[, "adj.P.Val"])
        avg.fold.change <- avg.fold.change + results[, "logFC"]
    }
    ...
    for (results in deg.results) {
        fold.change.signs <- fold.change.signs + sign(results[, "logFC"])
    }
